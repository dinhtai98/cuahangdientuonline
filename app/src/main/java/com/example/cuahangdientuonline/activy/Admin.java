...
import com.example.cuahangdientuonline.adapter.AdminAdapter;
import com.example.cuahangdientuonline.model.Adminmd;
                ...
                if(tendn.getText().length() != 0 && pw.getText().length() != 0){
                    for (int i = 0; i<mangadmin.size();i++) {
                        if (tendn.getText().toString().trim().equals(mangadmin.get(i).username) && pw.getText().toString().trim().equals(mangadmin.get(i).password)){
                            ...
                            CheckConnection.ShowToast_Short(getApplicationContext(),"Đăng nhập không thành công!");
                        }
                    }
                }
                else {
